...
static void edit_params(u32 argc, char **argv, char **envp) {
  ...
  while (--argc) {
    ...
    if (strstr(cur, "afl-igorfuzz-rt")) continue;
  ...
  if ((asan_set || getenv("AFL_USE_ASAN")) && \
      !getenv(IGORFUZZ_CALLSTACK_ENV_SHUTDOWN)) {
    ...
    switch (bit_mode) {
      ...
      case 0:
        if (!shared_linking && !partial_linking)
          cc_params[cc_par_cnt++] =
              alloc_printf("%s/afl-igorfuzz-rt.o", obj_path);
        break;
      ...
      case 32:
        if (!shared_linking && !partial_linking) {
          ...
          cc_params[cc_par_cnt++] =
              alloc_printf("%s/afl-igorfuzz-rt-32.o", obj_path);
      ...
      case 64:
        if (!shared_linking && !partial_linking) {
          ...
          cc_params[cc_par_cnt++] =
              alloc_printf("%s/afl-igorfuzz-rt-64.o", obj_path);
          if (access(cc_params[cc_par_cnt - 1], R_OK))
...
#undef NATIVE_MSG
  ...
  ptr = find_object("afl-igorfuzz-rt.o", argv[0]);

  if (!ptr) {

    FATAL(
        "Unable to find 'afl-igorfuzz-rt.o'. Please set the AFL_PATH "
        "environment variable.");

  }

  if (debug) { DEBUGF("afl-igorfuzz-rt=%s obj_path=%s\n", ptr, obj_path); }
